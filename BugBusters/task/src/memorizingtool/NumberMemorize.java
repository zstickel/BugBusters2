 ...
 */
public class NumberMemorize implements AutoCloseable {
    ...
    void Run() throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        try (Scanner scanner = new Scanner(System.in)) {  // Auto-close scanner
            while (!finished) {
                args = new ArrayList<>();  // Recreate args per iteration to free old references
                ...
                } catch (NoSuchMethodException | ArithmeticException | NullPointerException |
    ...
    void readFile(String path) throws IOException {
        ...
        try {
            List<Integer> list2 = readerThread.read(path);
            for (int i : list2) {
                list.add(i);
            }
    ...
    void subtract(int i, int j) {
        if(i>= list.size() || j>= list.size()){
            ...
            BigInteger a = BigInteger.valueOf(list.get(i)), b = BigInteger.valueOf(list.get(j));
    ...
    void pow(int i, int j) {
        ...
        if(i >= list.size() || j>= list.size()){
            ...
            BigInteger base = BigInteger.valueOf(list.get(i));
            BigInteger res;
            if (list.get(j) < 0) {
                int positiveExponent = -list.get(j);
                BigInteger positivePower = base.pow(positiveExponent);
                BigDecimal result = BigDecimal.ONE.divide(new BigDecimal(positivePower));
                System.out.printf("Calculation performed: %d ^ %d = %.6f\n", base, list.get(j), result);
            } else {
                res = base.pow(list.get(j));
                System.out.printf("Calculation performed: %d ^ %d = %d\n", base, list.get(j), res);
            }
        }
    }
    ...
    void factorial(int index) {
        if(index>=list.size()){
            ...
            if (list.get(index) < 0) {
                ...
                long res = 1;
                int i = 2;
                if (!(list.get(index) == 1 || list.get(index) == 0)) {
                    do {
                        res = res * (i++);
    ...
    void average() {
        ...
        System.out.printf("Average of all elements: %.6f", sum / (double) list.size());
        System.out.println();
    }
}
